...
class cOscLimit():
    ...
    def ReadValue(self,section,key):
        try:
            if section not in self.OscConfigureDic.keys():
                return ""
            if key not in self.OscConfigureDic[section].keys():
                return ""

            return self.OscConfigureDic[section][key]
            pass
        except Exception as e:
